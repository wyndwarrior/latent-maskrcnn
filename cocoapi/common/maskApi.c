...
void rleIou( RLE *dt, RLE *gt, siz m, siz n, byte *iscrowd, double *o, int i_only) {
  ...
  for( g=0; g<n; g++ ) for( d=0; d<m; d++ ) if(o[g*m+d]>0) {
    ...
    if(i==0 || i_only==1) u=1; else if(crowd) rleArea(dt+d,1,&u);
...
void rleNms( RLE *dt, siz n, uint *keep, double thr ) {
  ...
  for( i=0; i<n; i++ ) if(keep[i]) {
    for( j=i+1; j<n; j++ ) if(keep[j]) {
      rleIou(dt+i,dt+j,1,1,0,&u,0);
      if(u>thr) keep[j]=0;
    }
  }
}

void bbIou( BB dt, BB gt, siz m, siz n, byte *iscrowd, double *o, int i_only) {
  ...
  for( g=0; g<n; g++ ) {
    ...
    for( d=0; d<m; d++ ) {
      ...
      i=w*h; u = crowd ? da : da+ga-i; u = i_only == 1 ? 1.0 : u; o[g*m+d]=i/u;
    }
  }
}
