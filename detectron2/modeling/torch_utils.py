...
def Nonlinearity(nonlin):
    return {None: Identity, "relu": nn.ReLU, "leaky_relu": functools.partial(nn.LeakyReLU, negative_slope=0.1)}[
        nonlin
    ]()
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == None:
            ...
            xs = self.shortcut(x)
            xc1, xc2 = torch.split(xc, self.dim_out, dim=1)
            return torch.addcmul(xs, tensor1=xc1, tensor2=torch.sigmoid(xc2))
...
class SpecialSequential(nn.Sequential):
    def forward(self, x, **kwargs):
        for module in self._modules.values():
            signature = inspect.signature(module.forward)
            kw = {k: v for k, v in kwargs.items() if k in signature.parameters}
            x = module(x, **kw)
        return x
...
def initialize(module, initializers=None, scale=1.0):
    ...
    for name, var in module.named_parameters():
        ...
        if mode == "orthogonal":
            ...
            if len(var.shape) > 1:
                nn.init.orthogonal_(var, gain=scale)
            else:
